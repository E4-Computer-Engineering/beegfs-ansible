"""Testinfra tests for beegfs-ansible.mon role with BeeGFS v7."""
...
def test_beegfs_mon_tcp_port_listening(host):
    """Verify that BeeGFS monitoring service is listening on default TCP port."""
    socket = host.socket(f"tcp://0.0.0.0:{MON_TCP_PORT}")
    assert socket.is_listening


def test_beegfs_mon_udp_port_listening(host):
    """Verify that BeeGFS monitoring service is listening on default UDP port."""
    socket = host.socket(f"udp://0.0.0.0:{MON_UDP_PORT}")
    assert socket.is_listening


def test_beegfs_mon_listening_all_interfaces(host):
    """Verify that BeeGFS monitoring service is listening on all interfaces (default)."""
    # When no specific interfaces are configured, service should listen on 0.0.0.0
    tcp_socket = host.socket(f"tcp://0.0.0.0:{MON_TCP_PORT}")
