"""Testinfra tests for beegfs-ansible.metad role with BeeGFS v8 and custom configuration."""
...
METAD_TCP_PORT = "9005"
METAD_UDP_PORT = "9005"
METAD_INTERFACES = ["eth0"]


def test_beegfs_metad_service_running(host):
    """Verify that BeeGFS metadata server service is running with custom target_id."""
...
def test_beegfs_metad_tcp_port_listening(host):
    """Verify that BeeGFS metadata server is listening on custom TCP port."""
    socket = host.socket(f"tcp://0.0.0.0:{METAD_TCP_PORT}")
    assert socket.is_listening


def test_beegfs_metad_udp_port_listening(host):
    """Verify that BeeGFS metadata server is listening on custom UDP port."""
    socket = host.socket(f"udp://0.0.0.0:{METAD_UDP_PORT}")
    assert socket.is_listening


def test_beegfs_metad_interface_binding(host):
    """Verify that BeeGFS metadata server is bound to specified interfaces."""
    # Check that the service is listening on the specified interface
    for interface in METAD_INTERFACES:
        ...
        if cmd.rc == 0 and cmd.stdout.strip():
            ...
            tcp_socket = host.socket(f"tcp://{ip_address}:{METAD_TCP_PORT}")
            assert tcp_socket.is_listening, f"Service not listening on {ip_address}:{METAD_TCP_PORT}"
